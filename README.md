...
A token is strings of cryptic text, which can be stored on the client-side using local storage or session storage and is issued by the server as JWT (JSON Web Token) as the authentication token.  The server running these services can issue a token, in exchange for correct login credentials.
Token is used to implement protected routes and redirect routes.

- [ ] What steps can you take in your web apps to keep your data secure?
Web apps use tokens, generated by the server to authenticate for data exchange.
Web apps use cookies, generated by the browser to store state information.
Back up the data always.
Scan the Website for Vulnerabilities Often.
Keep Everything Up to Date.
Sanitize the User Output.
Use Authorized APIs Only.
Use strong passwords for login pages.
Protecting the data against viruses by running anti-virus software.

- [ ] Describe how web servers work.
The physical computer device is a web server, connected to the internet, and stores the code for different websites to be shared across the world at all times. It's role is to "serve” web pages it retrieves from your project code to users upon request. Hosting a website/app is loading the code for websites, or web apps, on a server.

- [ ] Which HTTP methods can be mapped to the CRUD acronym that we use when interfacing with APIs/Servers.
CREATE: axios.post
READ:  retrieve or fetch: axios.get
UPDATE: update or patch: axios.put
DELETE: delete or remove: axios.delete
